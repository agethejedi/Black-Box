...
export async function onRequestGet(context) {
  ...
  if (!env.BLACKBOX_AUDIO) return new Response(JSON.stringify({ error: "BLACKBOX_AUDIO not configured" }), { status: 503 })

  try {
    const obj = await env.BLACKBOX_AUDIO.get(key)
    if (!obj) return new Response(JSON.stringify({ error: "File not found" }), { status: 404 })

    const mimeType = obj.httpMetadata?.contentType || "audio/webm"
