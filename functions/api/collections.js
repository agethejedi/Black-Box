...
async function runCollectionAnalysis(env, collectionId, conversations, version) {
  try {
    const combined = conversations.map((c, i) =>
      `--- CONVERSATION ${i + 1} [${(c.created_at || "").slice(0, 10)}] [${c.source_type || "text"}] ---\n${c.raw_text || "[No transcript]"}`
    ).join("\n\n")
    ...
    `).bind(
      ...
      a.quality_score_avg || 0, a.escalation_score_avg || 0,
      a.validation_score_avg || 0, a.collaboration_score_avg || 0,
      a.topic_drift_score_avg || 0, a.resolution_probability_avg || 0,
